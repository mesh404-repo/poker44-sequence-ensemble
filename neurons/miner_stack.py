"""Poker44 SN126 miner — stacked meta-learner bot detector.

Serves P(focus player is a bot) per chunk using the p44xf detector: the recent-regime
gradient-boosted fleet plus two public reference models, blended with TWO decorrelated sequence
models over the hero-context action order — a bidirectional GRU and a hierarchical set-transformer.
Independent implementation kept separate from the baseline, depth-matched, and GRU-only detectors.
"""
import os
import sys
import time
...
from p44stack.detector import Detector
...
def _declared_implementation(root: Path):
    ...
    return [
        root / "p44stack" / "detector.py",
        root / "neurons" / "miner_stack.py",
    ]
...
class Miner(BaseMinerNeuron):
    def __init__(self, config=None):
        ...
        bt.logging.info("🃏 p44stack stacked meta-learner detector ready")

        self.model_manifest = build_local_model_manifest(
            repo_root=_ROOT,
            implementation_files=_declared_implementation(_ROOT),
            defaults={
                "model_name": os.getenv("POKER44_MODEL_NAME", "poker44-stacked-ensemble"),
                "model_version": os.getenv("POKER44_MODEL_VERSION", "4"),
                "framework": "stacked-gbdt-set-transformer-metalearner",
                "license": "MIT",
                "repo_url": os.getenv("POKER44_MODEL_REPO_URL", ""),
                "repo_commit": os.getenv("POKER44_MODEL_REPO_COMMIT", _head_commit(_ROOT)),
                "notes": (
                    "Stacked ensemble of five gradient-boosted base learners over engineered "
                    "features plus a hierarchical set-transformer over raw chunks, fused by a "
...
if __name__ == "__main__":
    with Miner() as node:
        bt.logging.info("Poker44 stacked-ensemble miner running.")
        while True:
            try:
                mg = node.metagraph
