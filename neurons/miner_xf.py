"""Poker44 SN126 miner — transformer sequence-ensemble bot detector.
...
from p44xf.detector import Detector
...
class Miner(BaseMinerNeuron):
    def __init__(self, config=None):
        ...
        bt.logging.info("🃏 p44xf transformer sequence-ensemble detector ready")

        self.model_manifest = build_local_model_manifest(
            repo_root=_ROOT,
            implementation_files=_declared_implementation(_ROOT),
            defaults={
                "model_name": os.getenv("POKER44_MODEL_NAME", "poker44-transformer-ensemble"),
                "model_version": os.getenv("POKER44_MODEL_VERSION", "2"),
                "framework": "gradient-boosting-plus-gru-plus-transformer",
                "license": "MIT",
                ...
                "notes": (
                    ...
                    "bidirectional-GRU and a hierarchical set-transformer over hero-context "
                    "action order (depth-capped to 48 even-sampled hands)."
                ),
                "open_source": True,
...
if __name__ == "__main__":
    with Miner() as node:
        bt.logging.info("Poker44 transformer sequence-ensemble miner running.")
        while True:
            try:
                mg = node.metagraph
